...
class HealthHistoryWeightPage extends StatefulWidget {
  const HealthHistoryWeightPage({Key? key}) : super(key: key);

  @override
  State<HealthHistoryWeightPage> createState() =>
      _HealthHistoryWeightPageState();
}

class _HealthHistoryWeightPageState extends State<HealthHistoryWeightPage> {
  var userUid = "";
  var userName = "";
  var healthData = [];
  ...
  retrieveData() async {
    ...
    for (var queryDocumentSnapshot in querySnapshot.docs) {
      ...
      healthData.add(
          {'createdAt': formattedDateTime, 'value': data['Weight'].toString()});

      healthValues.add(_HealthData(
          formattedDateTime, double.parse(data['Weight'].toString())));
    }
    setState(() {
                ...
                if (healthValues.isNotEmpty)
                  SingleChildScrollView(
                    ...
                    child: SfCartesianChart(
                        ...
                        // Chart title
                        title: ChartTitle(text: 'WEIGHT'.tr()),
                        // Enable legend
