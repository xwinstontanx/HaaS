...
class HealthHistoryHeightPage extends StatefulWidget {
  const HealthHistoryHeightPage({Key? key}) : super(key: key);

  @override
  State<HealthHistoryHeightPage> createState() =>
      _HealthHistoryHeightPageState();
}

class _HealthHistoryHeightPageState extends State<HealthHistoryHeightPage> {
  var userUid = "";
  var userName = "";
  var healthData = [];
  ...
  retrieveData() async {
    ...
    for (var queryDocumentSnapshot in querySnapshot.docs) {
      ...
      healthData.add(
          {'createdAt': formattedDateTime, 'value': data['Height'].toString()});

      healthValues.add(_HealthData(
          formattedDateTime, double.parse(data['Height'].toString())));
    }
    setState(() {
                ...
                if (healthValues.isNotEmpty)
                  SingleChildScrollView(
                    ...
                    child: SfCartesianChart(
                        ...
                        // Chart title
                        title: ChartTitle(text: 'HEIGHT'.tr()),
                        // Enable legend
